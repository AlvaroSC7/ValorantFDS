...
class RoulettePool:
    ...
    def resetPool(self):
        ...
        _save_json(contentData, jsonName= "resetPool")
        error = contentData.get('errors')
        if ((error is not None) and (error[0]['status'] != 200)):
            content = api.get_content(locale= None)     # Check generic call if es-ES does not work (locale is faster)
            contentData = content.json()
            _save_json(contentData, jsonName= "resetPoolGeneric")
...
def get_last_match_data(region: str, name: str, tag: str, target_player: str, target_team: str) -> str:
    ...
    if (type(target_team) is str):    # Normalize only if it is not None
        target_team = target_team.lower()
    if (target_player is None):
        response = errorCode.handleErrorCode(errorCode.ERR_CODE_122)
    elif (target_team is not None and target_team != "enemy" and target_team != "ally"):
...
def get_any_player_info(region: str, name: str, tag: str) -> str:
    ...
    if (errorCode.isErrorCode(allElo) is True):
        response = errorCode.handleErrorCode(allElo)
    else:
        if (allElo['rank'] is None or allElo['elo'] is None):
            eloStr = "Unranked"
        else:
...
def get_this_season_elo(region: str, name: str, tag: str) -> str:
    ...
    # Player is unranked, get level at least
        ...
        if (errorCode.isErrorCode(level) is True):
...
def get_avg_elo(region: str, name: str, tag: str) -> str:
    ...
    for team in player_and_opposite_team:
        ...
        for player in lastGame['players'][team]:
            ...
            if (errorCode.isErrorCode(elo) is True):
                return elo  # Return error code
            elif (elo is None):  # Unrated player, skip for calculation
                continue
            else:
                eloNum = elo[1]
...
def peak_elo(region: str, name: str, tag: str, target_player: str, targetTeam: str = None) -> dict:
    """
        ...
        """
    # Check first given command
    if (type(targetTeam) is str):    # Normalize only if it is not None
        targetTeam = targetTeam.lower()
    if (target_player is None):
        response = errorCode.handleErrorCode(errorCode.ERR_CODE_122)
    elif (targetTeam is not None and targetTeam != "enemy" and targetTeam != "ally"):
        response = errorCode.handleErrorCode(errorCode.ERR_CODE_123)
    else:
        # Check if target is player name or player character
        normalized_target_player = _normalize_agent_map(target_player)
        ...
        if (target_type == "map"):
            ...
            # Get elo and HS of the player controlling the selected agent in last player's game
            targetData = _get_last_match_agent_peak_elo(region= region, name= name, tag= tag,
                                                        targetAgent= normalized_target_player, targetTeam= targetTeam)
            isErrorCode = errorCode.handleErrorCode(targetData)
            if (isErrorCode is not None):
...
def get_vct(competition: str, team: str = None) -> str:
    """
        ...
        """     # noqa: E501 - Message for documentation. Code is readable enough
    # Check if team and competition are valid
    availableCompetitions = ["vct_americas", "challengers_na", "game_changers_na", "vct_emea", "vct_pacific",
                             "challengers_br", "challengers_jpn", "challengers_kr", "challengers_latam", "challengers_latam_n",
                             "challengers_latam_s", "challengers_apac", "challengers_sea_id", "challengers_sea_ph", "challengers_sea_sg_and_my",
                             "challengers_sea_th", "challengers_sea_hk_and_tw", "challengers_sea_vn", "valorant_oceania_tour",
                             "challengers_south_asia", "game_changers_sea", "game_changers_series_brazil", "game_changers_east_asia",
                             "game_changers_emea", "game_changers_jpn", "game_changers_kr", "game_changers_latam", "game_changers_championship",
                             "masters", "last_chance_qualifier_apac", "last_chance_qualifier_east_asia", "last_chance_qualifier_emea",
                             "last_chance_qualifier_na",  "last_chance_qualifier_br_and_latam", "vct_lock_in", "champions", "vrl_spain",
                             "vrl_northern_europe", "vrl_dach", "vrl_france", "vrl_east", "vrl_turkey", "vrl_cis", "mena_resilence",
    ...
    elif (team is not None and team not in available_teams[competition]):
...
def _get_target_type(target: str) -> str:
    ...
    if ((error is not None) and (error[0]['status'] != 200)):
        content = api.get_content(locale= None)     # Check generic call if es-ES does not work (locale is faster)
        contentData = content.json()
        _save_json(contentData, jsonName= "_get_target_type_generic")
...
def _get_elo(region: str, name: str, tag: str) -> tuple:
    ...
    if (eloData['data']['currenttierpatched'] is not None and eloData['data']['elo'] is not None):
...
def _get_last_match_agent_peak_elo_old_game(name: str, gameId: str, targetAgent: str, targetTeam: str = None) -> dict:
    ...
    if (type(player_and_opposite_team) is str and errorCode.isErrorCode(player_and_opposite_team) is True):
    ...
    if (matchData['data'][0]['metadata']['mode_id'] == "deathmatch"):
        ...
        if (errorCode.isErrorCode(targetData) is True):
        ...
        if ((targetData is None and targetTeam is None) or (targetTeam == "ally")):
    ...
    if (targetData is None):
        ...
        if (errorCode.isErrorCode(peak_elo) is True):
...
def _get_last_match_agent_peak_elo(region: str, name: str, tag: str, targetAgent: str, targetTeam: str = None) -> dict:
    ...
    if (len(matchData['data']) == 0):    # If there are no matches try another version of the API that allows older games
        ...
        if (gameId is None):
            ...
            return _get_last_match_agent_peak_elo_old_game(name= name, gameId= gameId, targetAgent= targetAgent,
                                                           targetTeam= targetTeam)   # GameID v2 API support older games.
...
def _get_last_match_user_peak_elo(region: str, name: str, tag: str, targetName: str) -> dict:
    ...
    if (errorCode.isErrorCode(targetTag) is True):
        return targetTag
...
def _get_last_match_agent_data(region: str, name: str, tag: str, targetAgent: str, targetTeam: str = None) -> dict:
    ...
    if (len(matchData['data']) == 0):
        ...
        if (gameId is None):
            ...
            return _get_last_match_agent_data_old_game(name= name, gameId= gameId, targetAgent= targetAgent,
                                                       targetTeam= targetTeam)   # GameID v2 API support older games.
    ...
    if (targetData is None):
        ...
        # Get data of the desired player
        result = _extract_last_game_info(region= region, name= targetData['name'], tag = targetData['tag'],
...
def _get_last_match_player_data(region: str, name: str, tag: str, targetName: str) -> dict:
    ...
    if (errorCode.isErrorCode(playerTag) is True):
        return playerTag
    else:
        # Get data of the desired player
...
def _extract_last_game_info(region: str, name: str, tag: str, mode_id: str) -> str:
    ...
    if (errorCode.isErrorCode(target_elo) is True):
        return target_elo   # Return error code
    elif (errorCode.isErrorCode(target_HS) is True):
        return target_HS    # Return error code
    elif (errorCode.isErrorCode(target_peak) is True):
